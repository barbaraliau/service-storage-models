 ...
 * @param {string} options.name - processor
...
PaymentProcessors.statics.create = function(options) {
  ...
  return new Promise((resolve, reject) => {
    ...
    PaymentProcessors.findOne({ user: email }).then((doc) => {
      if (doc) {
        processor = doc;
      } else {
...
PaymentProcessors.statics.get = function(options) {
  ...
  return new Promise((resolve, reject) => {
    ...
    PaymentProcessors.find({ user: email }).then((paymentProcessor) => {
      ...
      if (!name) {
        return reject(new errors.BadRequestError(
          'Must pass in processor')
        );
      }

      if (name === 'default') {
        const processor = paymentProcessor.default;
        return processor === _valid(processor)
          ? resolve([{ [processor]: paymentProcessor[processor] }])
          : resolve(null);
      }
 ...
 */
PaymentProcessors.methods.setDefaultPaymentProcessor = function(options) {
  const processor = this;
  const name = options.name;
