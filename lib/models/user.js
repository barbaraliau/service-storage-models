...
User.methods.addPaymentProcessor = function(name, data) {
  ...
  return adapter.register(data, User.email).then((processorData) => {
    const updatedProcessors = User.paymentProcessors.concat([{
      name: name,
      default: true,
      rawData: adapter.serializeData(processorData)
    }]);

    User.paymentProcessors = updatedProcessors;
