...
User.virtual('defaultPaymentProcessor')
  .get(function() {
    const defaultPaymentProcessor = this.paymentProcessors.find((processor) => {
 ...
 * @param {String} data - user payment token
...
User.methods.addPaymentProcessor = function(name, data) {
  ...
  // ! this cannot work. if data is a token, then how can you update
  // a paymentProcessor with it?
  if (!!existingProcessor) {
    // return existingProcessor.update(existingProcessor, data);
    return User.updatePaymentProcessor(adapter, existingProcessor,
       data);
  }
...
User.methods.updatePaymentProcessor = function(adapter, processor,
data) {
  console.log('updatePaymentProcessor')
  return adapter
    .validate(processor)
    .then((isValid) => {
      if (!isValid) {
        return new errors.BadRequestError('Adapter is not valid');
      }
